...
int main(){
    ...
    for (int i = 0; i < students; i++) {
        ...
        for (int j = 0; j < electives_per_student; j++) {   // creates elective values for each student
            int randNum = rand() % electives + 1;           // generates a random number within random elective bounds
            for (int k = 0; k < j-1; k++) {                 // iterates through all already-generated indicies
                if (randNum == *(electiveList+k)) {         // checks the indicies to make sure the same elective isn't generated twice
                    j--;
